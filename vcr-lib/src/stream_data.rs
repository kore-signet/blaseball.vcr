...
use crate::*;
use vcr_lookups::{DATES_TO_GAMES, GAME_ID_TABLE};
use vcr_schemas::*;
...
macro_rules! fetch_batch {
    ...
    ($what:ty; from $db:expr; with $ids:expr; at $at:expr; clamped) => {
        fetch_batch!($what; from $db; with $ids; at clamp($at, 1_599_169_238_000_000_000, i64::MAX))
    };
...
fn games_by_date_and_time(
    ...
    if let Some(ids) = DATES_TO_GAMES.get(&date.to_bytes()) {
        let game_ids: Vec<[u8; 16]> = ids
            .iter()
            .map(|tag| (GAME_ID_TABLE.inverter[*tag as usize]).into_bytes())
            .collect();
...
fn games_for_bets(
    ...
    if let Some(ids) = DATES_TO_GAMES.get(&date.to_bytes()) {
        ...
        'outer: for id in game_ids {
            ...
            while game.data.away_odds == Some(0f64) && game.data.home_odds == Some(0f64) {
...
pub fn stream_data(db: &DatabaseManager, at: i64) -> VCRResult<StreamDataWrapper> {
    let sim = db
    ...
    let schedule: Vec<GameUpdate> = if sim.phase == 14 && working_date.season == 22 {
        if let Some(data) = db
            .get_entity::<GameUpdate>(
                uuid::uuid!("d162b23a-9832-4e78-8d78-5d131393fd61").as_bytes(),
                at,
            )?
            .map(|v| v.data)
        {
    ...
    if let Some(ref tourn) = tournament {
        ...
        match sim.playoffs {
            Some(vcr_schemas::sim::Playoffs::String(ref id)) => {
                postseason = Some(playoffs(db, id.as_bytes(), sim.play_off_round, at)?);
            }
            Some(vcr_schemas::sim::Playoffs::StringArray(ref ids)) => {
